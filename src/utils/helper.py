...
def prepare_data_module_params(dataset_names, config):
    ...
    for dataset_name in dataset_names:
        data_modules_params[dataset_name] = prepare_data_dicts(
...
def prepare_data_dicts(dataset_name, config):
    ...
    elif dataset_name in ["dipco", "santa_barbara"]:
        test_data_dict = {"cut_set_path": config["test_cut_set_path"]}
...
def get_cuts_subset(cut_set_path: Pathlike, cut_subset_size: int) -> CutSet:
    ...
    NUM_CUTS_PER_HOUR = (
        60 * 60 // 5
    )  # 720 cuts in 1 hr (as duration of each cut is 5 secs)

    original_cuts_len = len(cuts)
    original_cuts_total_time = original_cuts_len * 5

    subset_cuts_total_time = cut_subset_size * 60 * 60  # subset size in seconds

    valid_cuts_total_time = min(original_cuts_total_time, subset_cuts_total_time)

    if valid_cuts_total_time < subset_cuts_total_time:
        print(
            "Warning: Subset size is larger than the original cut set size. Using the original cut set size instead."
        )
        # cuts.to_file(
        #     str(cut_set_path).replace(
        #         ".jsonl.gz", f"_subset_{cut_subset_size}.jsonl.gz"
        #     )
        # )
        return cuts
    ...
    # new_cut_set_path = str(cut_set_path).replace(
    #     ".jsonl.gz", f"_subset_{cut_subset_size}.jsonl.gz"
    # )
    # cuts_subset.to_file(new_cut_set_path)

    return cuts_subset  # inefficient, but works for now
