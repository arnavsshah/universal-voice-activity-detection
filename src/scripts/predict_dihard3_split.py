...
def predict_dihard3_split(**kwargs):
    ...
    if dataset_name == 'dihard3':
        ...
        for domain in domains:
            kwargs['dihard3']['test_cut_set_path'] = f'/export/c01/ashah108/vad/data/dihard3/manifests/domains/eval_cuts_feats_ssl_{domain}.jsonl.gz'
            ...
            torch.save(test_preds, os.path.join(kwargs['predict_output_dir'], f'test_preds_{dataset_name}_{domain}.pt'))
...
def get_new_cuts(dataset_name,
    ...
    recording_duration = [{'recording_id': obj['id'], 'duration': obj['duration']} for obj in recordings if recording_domain[obj['id']] == domain]
    recording_tensor = []
    ...
    for duration_obj in recording_duration:
        ...
        start = end
        end += math.ceil(duration_obj['duration'] / 0.02) + 1
    ...
    for i, obj in enumerate(recording_tensor):
        ...
        # print(obj['recording_id'], intervals)
        ...
        # print(obj['recording_id'], window_intervals)
    ...
    print(f"Dataset: {dataset_name}, domain {domain}, Buffer: {buffer}, Phase: {phase}")
    print("\n")
