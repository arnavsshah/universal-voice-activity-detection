...
class GlobalDataModule(pl.LightningDataModule):
    def __init__(
        ...
        enable_musan: bool = False,
        musan_cut_set_path: Optional[Pathlike] = None,
    ):
        ...
        enable_musan : bool, optional
            Add MUSAN noise to training dataset, by default False
        musan_cut_set_path : Optional[Pathlike], optional
            Path to MUSAN cut set, by default None. Required if enable_musan is True
        """
        ...
        self.enable_musan = enable_musan

        if enable_musan:
            assert musan_cut_set_path, f'musan_cut_set_path must have a valid value as enable_musan is True. Currently, it is None'
            self.musan_cut_set_path = musan_cut_set_path
    ...
    def setup(self, stage: str = None):
        for key, value in self.data_modules_params.items():
            ...
            if key == 'dipco':
                ...
                self.test_cuts_all.append(get_dipco_cut(**value[2]))
    ...
    def train_dataloader(self):
        ...
        if self.enable_musan:
            musan_cuts = get_musan_cut(self.musan_cut_set_path)
            cut_transforms.append(CutMix(musan_cuts.to_eager(), snr=(10, 20), p=0.5, preserve_id="left"))
            
        dataset = VadDataset(cut_transforms=cut_transforms)
        sampler = SimpleCutSampler(self.train_cuts, max_duration=self.max_duration, shuffle=True, drop_last=True)
        return DataLoader(dataset, sampler=sampler, batch_size=None, num_workers=4)

    def val_dataloader(self):
        dataset = VadDataset()
        sampler = SimpleCutSampler(self.dev_cuts, max_duration=self.max_duration, drop_last=True)
        return DataLoader(dataset, sampler=sampler, batch_size=None, num_workers=4)
