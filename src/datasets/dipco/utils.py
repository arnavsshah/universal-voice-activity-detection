...
def create_dipco_dataset(
    ...
    **kwargs
    ...
    output_dir = Path(output_dir) if output_dir else Path('/export/c01/ashah108/vad/data/dipco/manifests')
...
def create_dipco_cut(
    ...
    **kwargs
    ...
    output_dir = Path(output_dir) if output_dir else Path('/export/c01/ashah108/vad/data/dipco/manifests')
    feats_dir = Path(feats_dir) if feats_dir else Path('/export/c01/ashah108/vad/data/dipco/feats')
    ...
    if phase == "dev":
        ...
        train_cuts.to_file(f'{output_dir}/train_{mic}_cuts_{feature_extractor}.jsonl.gz')
        dev_cuts.to_file(f'{output_dir}/dev_{mic}_cuts_{feature_extractor}.jsonl.gz')

        return train_cuts, dev_cuts
    
    else:
        cuts.to_file(f'{output_dir}/test_{mic}_cuts_{feature_extractor}.jsonl.gz')
        # cuts.describe()
