...
def get_callhome_english_cut(cut_set_path: Optional[Pathlike]) -> lhotse.CutSet:
    ...
    return load_callhome_english_cut(cut_set_path)


def load_callhome_english_cut(cut_set_path: Pathlike) -> lhotse.CutSet:
...
def create_callhome_english_dataset(
    ...
    **kwargs,
    ...
    output_dir = (
        Path(output_dir)
        if output_dir
        else Path("/export/c01/ashah108/vad/data/callhome_english/manifests")
    )
...
def create_callhome_english_cut(
    ...
    **kwargs,
    ...
    assert phase in [
        ...
        "devtest",
        "evaltest",
    ], f"Invalid phase: {phase}. Must be one of ['train', 'devtest', 'evaltest']"
    ...
    feats_dir = (
        Path(feats_dir)
        if feats_dir
        else Path("/export/c01/ashah108/vad/data/callhome_english/feats")
    )
    ...
    rec = load_manifest_lazy(
        f"{output_dir}/callhome-english_recordings_{phase}.jsonl.gz"
    )
    sup = load_manifest_lazy(
        f"{output_dir}/callhome-english_supervisions_{phase}.jsonl.gz"
    )
