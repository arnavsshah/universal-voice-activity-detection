...
def create_fisher_english_dataset(
    corpus_dir: Pathlike,
    audio_dirs: Pathlike,
    transcript_dirs: Optional[Pathlike] = None,
    output_dir: Optional[Pathlike] = None,
    **kwargs
) -> None:

    output_dir = Path(output_dir) if output_dir else Path('/export/c01/ashah108/vad/data/fisher_english/manifests')
...
def create_fisher_english_cut(
    ...
    **kwargs
    ...
    output_dir = Path(output_dir) if output_dir else Path('/export/c01/ashah108/vad/data/fisher_english/manifests')
    feats_dir = Path(feats_dir) if feats_dir else Path('/export/c01/ashah108/vad/data/fisher_english/feats')
    ...
    rec = load_manifest_lazy(f'{output_dir}/fisher-english_recordings_{phase}.jsonl.gz')
    sup = load_manifest_lazy(f'{output_dir}/fisher-english_supervisions_{phase}.jsonl.gz')
