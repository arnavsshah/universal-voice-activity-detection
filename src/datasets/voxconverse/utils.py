...
def create_voxconverse_dataset(
    ...
    output_dir = (
        Path(output_dir)
        if output_dir
        else Path("/export/c01/ashah108/vad/data/voxconverse/manifests")
    )

    if not os.path.exists(output_dir):
        output_dir.mkdir(parents=True, exist_ok=True)

    prepare_voxconverse(corpus_dir=corpus_dir, output_dir=output_dir, split_test=True)
...
def create_voxconverse_cut(
    ...
    **kwargs,
    ...
    feats_dir = (
        Path(feats_dir)
        if feats_dir
        else Path("/export/c01/ashah108/vad/data/voxconverse/feats")
    )
