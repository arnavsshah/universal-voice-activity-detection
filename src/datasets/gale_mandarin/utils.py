...
from lhotse.recipes import prepare_gale_mandarin


Pathlike = Union[Path, str]


def get_gale_mandarin_cut(cut_set_path: Optional[Pathlike]) -> lhotse.CutSet:
    ...
    return load_gale_mandarin_cut(cut_set_path)


def load_gale_mandarin_cut(cut_set_path: Pathlike) -> lhotse.CutSet:
    return load_manifest_lazy(cut_set_path)


def create_gale_mandarin_dataset(
    audio_dirs: Pathlike,
    ...
    **kwargs
    ...
    output_dir = Path(output_dir) if output_dir else Path('/export/c01/ashah108/vad/data/gale_mandarin/manifests')

    if not os.path.exists(output_dir):
        output_dir.mkdir(parents=True, exist_ok=True)

    prepare_gale_mandarin(audio_dirs=audio_dirs, transcript_dirs=transcript_dirs, output_dir=output_dir)


def create_gale_mandarin_cut(
    output_dir: Optional[Pathlike] = None,
    ...
    **kwargs
    ...
    output_dir = Path(output_dir) if output_dir else Path('/export/c01/ashah108/vad/data/gale_mandarin/manifests')
    feats_dir = Path(feats_dir) if feats_dir else Path('/export/c01/ashah108/vad/data/gale_mandarin/feats')
    ...
    rec = load_manifest_lazy(f'{output_dir}/gale-mandarin_recordings_{phase}.jsonl.gz')
    sup = load_manifest_lazy(f'{output_dir}/gale-mandarin_supervisions_{phase}.jsonl.gz')
