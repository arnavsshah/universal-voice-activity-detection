...
def create_gale_mandarin_dataset(
    ...
    **kwargs,
    ...
    output_dir = (
        Path(output_dir)
        if output_dir
        else Path("/export/c01/ashah108/vad/data/gale_mandarin/manifests")
    )
...
def create_gale_mandarin_cut(
    ...
    **kwargs,
    ...
    feats_dir = (
        Path(feats_dir)
        if feats_dir
        else Path("/export/c01/ashah108/vad/data/gale_mandarin/feats")
    )
    ...
    rec = load_manifest_lazy(
        f"{output_dir}/gale-mandarin_recordings_{phase_temp}.jsonl.gz"
    )
    sup = load_manifest_lazy(
        f"{output_dir}/gale-mandarin_supervisions_{phase_temp}.jsonl.gz"
    )
