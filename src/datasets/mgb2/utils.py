...
def get_mgb2_cut(cut_set_path: Optional[Pathlike]) -> lhotse.CutSet:
    ...
    return load_mgb2_cut(cut_set_path)


def load_mgb2_cut(cut_set_path: Pathlike) -> lhotse.CutSet:
    return load_manifest_lazy(cut_set_path)


def create_mgb2_dataset(
    corpus_dir: Pathlike,
    ...
    **kwargs,
    ...
    output_dir = (
        Path(output_dir)
        if output_dir
        else Path("/export/c01/ashah108/vad/data/mgb2/manifests")
    )

    if not os.path.exists(output_dir):
        output_dir.mkdir(parents=True, exist_ok=True)

    prepare_mgb2(corpus_dir=corpus_dir, output_dir=output_dir)
...
def create_mgb2_cut(
    ...
    **kwargs,
    ...
    feats_dir = (
        Path(feats_dir)
        if feats_dir
        else Path("/export/c01/ashah108/vad/data/mgb2/feats")
    )
    ...
    rec = load_manifest_lazy(f"{output_dir}/mgb2_recordings_{phase}.jsonl.gz")
    sup = load_manifest_lazy(f"{output_dir}/mgb2_supervisions_{phase}.jsonl.gz")
