...
def get_aishell_cut(cut_set_path: Optional[Pathlike]) -> lhotse.CutSet:
    ...
    return load_aishell_cut(cut_set_path)


def load_aishell_cut(cut_set_path: Pathlike) -> lhotse.CutSet:
    return load_manifest_lazy(cut_set_path)


def create_aishell_dataset(
    corpus_dir: Pathlike,
    ...
    **kwargs,
    ...
    output_dir = (
        Path(output_dir)
        if output_dir
        else Path("/export/c01/ashah108/vad/data/aishell/manifests")
    )

    if not os.path.exists(output_dir):
        output_dir.mkdir(parents=True, exist_ok=True)

    prepare_aishell(corpus_dir=corpus_dir, output_dir=output_dir)


def create_aishell_cut(
    output_dir: Optional[Pathlike] = None,
    ...
    **kwargs,
    ...
    feats_dir = (
        Path(feats_dir)
        if feats_dir
        else Path("/export/c01/ashah108/vad/data/aishell/feats")
    )
    ...
    rec = load_manifest_lazy(f"{output_dir}/aishell_recordings_{phase}.jsonl.gz")
    sup = load_manifest_lazy(f"{output_dir}/aishell_supervisions_{phase}.jsonl.gz")
